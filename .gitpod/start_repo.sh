...
export DDEV_REPO=${DDEV_REPO:-https://github.com/mikastamm/weinert-industries.com}
...
if [ -d ${GITPOD_REPO_ROOT}/${reponame} ]; then
  source "$GITPOD_REPO_ROOT"/.gitpod/setup_vscode_git.sh
  ...
  if [ -f composer.json ]; then
    ...
    ddev composer install -d web/app/plugins/weinert-plugin/
  fi
